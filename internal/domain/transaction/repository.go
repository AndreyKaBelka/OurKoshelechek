...
const selectColumns = `id, group_id, type, amount, category_id, payer_mode, payer_user_id,
	recipient_user_id, date, comment, created_by, created_at, updated_at`
...
func (r *Repository) Create(ctx context.Context, t Transaction) error {
	...
	`,
		...
		t.RecipientUserID, t.Date, t.Comment, t.CreatedBy, t.CreatedAt, t.UpdatedAt,
	)
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, t Transaction) error {
	tag, err := r.db.Exec(ctx, `
		...
		SET type = $3, amount = $4, category_id = $5, payer_mode = $6, payer_user_id = $7,
			recipient_user_id = $8, date = $9, comment = $10, updated_at = $11
		WHERE group_id = $1 AND id = $2
	`,
		t.GroupID, t.ID, t.Type, t.Amount, t.CategoryID, t.PayerMode, t.PayerUserID,
		t.RecipientUserID, t.Date, t.Comment, t.UpdatedAt,
	)
	if err != nil {
...
func (r *Repository) List(ctx context.Context, groupID uuid.UUID, filter Filter, first int, after *uuid.UUID) ([]Transaction, bool, int, error) {
	...
	if filter.RecipientUserID != nil {
		add("recipient_user_id = $%d", *filter.RecipientUserID)
	}
...
// CategoryPayerAmount is one row of a SumAmountByCategoryAndPayer result.
type CategoryPayerAmount struct {
	CategoryID uuid.UUID
	UserID     uuid.UUID
	Amount     int64
}

// SumAmountByCategoryAndPayer combines SumAmountByCategory and
// SumAmountByPayer: for each (category, user) pair with at least one matching
// row, the amount of txType transactions attributed to that user (full amount
// for payer_mode = 'user', the user's share for 'split'). Uncategorized rows
// are excluded, as in SumAmountByCategory.
func (r *Repository) SumAmountByCategoryAndPayer(ctx context.Context, groupID uuid.UUID, txType Type, from, to time.Time) ([]CategoryPayerAmount, error) {
	rows, err := r.db.Query(ctx, `
		SELECT category_id, user_id, sum(amount) FROM (
			SELECT category_id, payer_user_id AS user_id, amount
			FROM transactions
			WHERE group_id = $1 AND type = $2 AND payer_mode = 'user' AND date >= $3 AND date < $4 AND category_id IS NOT NULL
			UNION ALL
			SELECT t.category_id, tps.user_id, tps.amount
			FROM transaction_payer_shares tps
			JOIN transactions t ON t.id = tps.transaction_id
			WHERE t.group_id = $1 AND t.type = $2 AND t.payer_mode = 'split' AND t.date >= $3 AND t.date < $4 AND t.category_id IS NOT NULL
		) x
		GROUP BY category_id, user_id
		ORDER BY category_id, user_id
	`, groupID, txType, from, to)
	if err != nil {
		return nil, fmt.Errorf("sum transaction amount by category and payer: %w", err)
	}
	defer rows.Close()

	var result []CategoryPayerAmount
	for rows.Next() {
		var cpa CategoryPayerAmount
		if err := rows.Scan(&cpa.CategoryID, &cpa.UserID, &cpa.Amount); err != nil {
			return nil, fmt.Errorf("scan category payer amount: %w", err)
		}
		result = append(result, cpa)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate category payer amounts: %w", err)
	}
	return result, nil
}

// TransferAmount is one row of a SumTransfersByMember result.
type TransferAmount struct {
	UserID uuid.UUID
	// Sent — сумма переводов, отправленных участником (его расход).
	Sent int64
	// Received — сумма переводов, полученных участником (его доход).
	Received int64
}

// SumTransfersByMember returns, for each user who sent or received at least
// one transfer in groupID with date in the half-open range [from, to), the
// total they sent (as payer_user_id) and received (as recipient_user_id).
func (r *Repository) SumTransfersByMember(ctx context.Context, groupID uuid.UUID, from, to time.Time) ([]TransferAmount, error) {
	rows, err := r.db.Query(ctx, `
		SELECT user_id, sum(sent), sum(received) FROM (
			SELECT payer_user_id AS user_id, amount AS sent, 0::bigint AS received
			FROM transactions
			WHERE group_id = $1 AND type = 'transfer' AND date >= $2 AND date < $3
			UNION ALL
			SELECT recipient_user_id, 0, amount
			FROM transactions
			WHERE group_id = $1 AND type = 'transfer' AND date >= $2 AND date < $3
		) x
		GROUP BY user_id
	...
	`, groupID, from, to)
	if err != nil {
		return nil, fmt.Errorf("sum transfers by member: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ta.UserID, &ta.Sent, &ta.Received); err != nil {
			return nil, fmt.Errorf("scan transfer amount: %w", err)
		}
		result = append(result, ta)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate transfer amounts: %w", err)
	}
	return result, nil
}
